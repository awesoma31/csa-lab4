...
`let if else while print read readInt list inter intOn intOff`.
...
- При использовании переменной транслятор ставит адрес значения если и генерирует установку значения по адресу переменной.
...
Детальное описание ISA [здесь](docs/isa.md), потактовое подробно описано здесь - [micro.go](pkg/machine/micro.go)
